...
$watchSettingsCopy = "Survivor Level inheritance is configured through sandbox settings and allows the host to set a percentage of a deceased character's Survivor Level that passes to that player's next eligible survivor. The optional digital watch percentage is enabled per client through Mod Options and shows Player 1's progress toward the next Survivor Level."
...
Assert-ReleaseCondition (@($descriptionLines -ceq $watchSettingsCopy).Count -eq 1) 'exact Markdown sandbox inheritance and per-client Mod Options watch copy'
Assert-ReleaseCondition (@($workshopLines -ceq "description=$watchSettingsCopy").Count -eq 1) 'exact Workshop sandbox inheritance and per-client Mod Options watch copy'
